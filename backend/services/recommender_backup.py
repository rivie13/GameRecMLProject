...
class HybridRecommender:
    ...
    def _predict_engagement(self, game_row: pd.Series, owned_games_df: pd.DataFrame) -> float:
        """Predict engagement score (0-100) for a game using the ML model."""
        try:
            # Build user profile features from owned games
            ...
            # Predict engagement (0-1 scale from model)
            ...
            logger.debug(f"ML prediction failed for game {game_row.get('name', 'unknown')}: {e}")
    ...
    def _build_user_features(self, owned_games_df: pd.DataFrame) -> Dict[str, float]:
        ...
        # Genre preferences (top 50 genres)
        all_genres = []
    ...
    def _build_game_features(self, game_row: pd.Series) -> Dict[str, float]:
        ...
        # Basic game stats
        features['game_price'] = float(game_row.get('price', 0)) / 100.0  # cents to dollars
        ...
        # Genres as binary features (top 50)
        ...
        # Tags as binary features (top 100)
    ...
    def generate_recommendations(
        ...
        if self.model is not None and self.scaler is not None and self.feature_names is not None:
            try:
                catalog_unowned['ml_score'] = catalog_unowned.apply(
                    lambda row: self._predict_engagement(row, owned_games_df),
                    axis=1
                )
                logger.info(f"  Generated ML predictions for {len(catalog_unowned)} games")
                logger.info(f"  ML score range: {catalog_unowned['ml_score'].min():.1f} - {catalog_unowned['ml_score'].max():.1f}")
            except Exception as e:
                logger.error(f"  ❌ ML prediction failed: {e}. Using fallback scores.")
                catalog_unowned['ml_score'] = 50.0
        else:
            logger.warning("  Using fallback ML scores (models not loaded)")
            catalog_unowned['ml_score'] = 50.0
        ...
        # Store the weights used for this recommendation run
    ...
    def explain_recommendation(
        ...
        if self.model is not None:
            try:
                ml_score = self._predict_engagement(game, owned_games_df)
            except Exception as e:
                logger.warning(f"ML prediction failed in explain: {e}")
                ml_score = 50.0
        else:
            ml_score = 50.0
